...
# 필요 조건: Python 3.8+, Claude Code CLI (claude)
# ============================================================================
set -euo pipefail
...
check_prerequisites() {
    ...
    if command -v claude &>/dev/null; then
        ...
        echo ""
        read -rp "$(echo -e "${YELLOW}Claude CLI 없이 계속 진행할까요? (y/N): ${NC}")" yn
        case "$yn" in
            [yY]*) warn "나중에 Claude CLI를 설치해야 봇이 작동합니다." ;;
            *)     echo "설치 후 다시 실행하세요."; exit 0 ;;
        esac
    fi
}
...
get_user_input() {
    ...
    echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo "1. @BotFather에서 봇을 만들고 토큰을 받으세요"
    echo "   Telegram → @BotFather → /newbot → 토큰 복사"
    echo ""
    echo "2. @userinfobot에서 Chat ID를 확인하세요"
    echo "   Telegram → @userinfobot → /start → ID 복사"
    echo ""
    ...
    while true; do
        ...
        err "유효하지 않은 토큰 형식입니다. (예: 123456789:ABCdef...)"
    done
    ...
    while true; do
        ...
        err "유효하지 않은 Chat ID입니다. (숫자만 입력)"
    done

    # Working directory
    DEFAULT_WORKDIR="$HOME"
    read -rp "$(echo -e "${CYAN}작업 디렉토리 [$DEFAULT_WORKDIR]: ${NC}")" WORK_DIR
    WORK_DIR="${WORK_DIR:-$DEFAULT_WORKDIR}"
    if [[ ! -d "$WORK_DIR" ]]; then
        err "디렉토리가 존재하지 않습니다: $WORK_DIR"
        exit 1
    fi
    ...
    echo ""
    read -rp "$(echo -e "${YELLOW}이 설정으로 설치할까요? (Y/n): ${NC}")" confirm
    case "$confirm" in
        [nN]*) echo "취소됨."; exit 0 ;;
    esac
}
...
EOF
    ...
    ok "systemd 서비스 등록 완료 (자동 시작)"
    echo ""
    echo "  상태 확인: systemctl --user status claude-telegram"
    echo "  로그 확인: cat $INSTALL_DIR/bot.log"
    echo "  중지:      systemctl --user stop claude-telegram"
    echo "  재시작:    systemctl --user restart claude-telegram"
}

setup_autostart_macos() {
    info "launchd 서비스 등록 중..."
...
EOF
    ...
    ok "launchd 서비스 등록 완료 (자동 시작)"
    echo ""
    echo "  상태 확인: launchctl list | grep claude"
    echo "  로그 확인: cat $INSTALL_DIR/bot.log"
    echo "  중지:      launchctl stop com.claude.telegram-bot"
    echo "  재시작:    launchctl stop com.claude.telegram-bot && launchctl start com.claude.telegram-bot"
}
...
setup_autostart_wsl() {
    ...
    # Start now
    eval "$start_cmd"
    ok "WSL 자동 시작 설정 완료 (.bashrc에 추가)"
    echo ""
    echo "  상태 확인: pgrep -f telegram-bot.py"
    echo "  로그 확인: cat $INSTALL_DIR/bot.log"
    echo "  중지:      pkill -f telegram-bot.py"
}
...
print_uninstall() {
    ...
    case "$OS" in
        ...
        wsl)
            ...
            echo "  .bashrc에서 '# claude-telegram-bot autostart' 줄 제거"
            ;;
    esac
...
main() {
    ...
    if [[ "$OS" == "unknown" || "$OS" == "windows-git-bash" ]]; then
        err "이 OS에서는 직접 지원되지 않습니다."
        echo "Windows는 WSL(Windows Subsystem for Linux)에서 실행하세요."
        exit 1
    fi
